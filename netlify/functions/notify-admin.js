...
exports.handler = async (event) => {
  ...
  try {
    ...
    const requestKey = isUrgentRequest ? String(payload.id) + ":" + (payload.urgentRequestedAt || 0) : "";
    if (isUrgentRequest) {
      const crypto = require("crypto");
      const hash = crypto.createHash("sha256").update(requestKey).digest("hex");
      delivery = admin.database().ref("/directorQueue/v1/urgentPushDeliveries/" + hash);
      ...
      if (!claim.committed) return json(200, { ok: true, sent: true, skipped: "duplicate-request" });
    }
    const eventId = isUrgentRequest ? "urgent-request:" + requestKey : "new-registration:" + payload.bookingId;
    const send = !isUrgentRequest && payload.bookingId
      ? message => require("./_notification").sendOnce(eventId, message)
      : message => admin.messaging().send(message);
    await send({
      token: adminToken,
      ...
      webpush: {
        ...
        notification: {
          ...
          ...(isUrgentRequest ? { tag: "urgent-request-" + requestKey, renotify: false } : {}),
      ...
      data: { ...(isUrgentRequest || payload.bookingId ? { eventId } : {}), title: notifTitle, body: notifBody, type: isUrgentRequest ? "urgent-request" : "new-registration", requestKey: isUrgentRequest ? `${payload.id}:${payload.urgentRequestedAt || 0}` : "" },
    });

    pushAccepted = true;
    if (delivery) await delivery.set({ sent: true, sentAt: Date.now() });
    return json(200, { ok: true, sent: true });
  } catch (error) {
    if (delivery && claimId && !pushAccepted) {
      try { await delivery.transaction(value => value?.claimId === claimId ? null : undefined); }
      catch (cleanupError) { console.error("urgent delivery cleanup failed", cleanupError); }
    }
