...
async function recordNewlyCompleted(prevBookings, nextBookings) {
  ...
  if (newlyCompleted.length === 0 && newlyRegistered.length === 0) return;

  const todayStr = new Date(Date.now() + 9 * 60 * 60 * 1000).toISOString().split("T")[0];
  const historyRef = admin.database().ref(HISTORY_PATH);

  await Promise.all(newlyRegistered.map(b => historyRef.push({
    type: "등록", category: b.department || b.category || "기타",
    reporter: b.reporterName || "", title: b.title || "", date: todayStr,
    bookingId: b.id, createdAt: Date.now()
  })));
  await Promise.all(
