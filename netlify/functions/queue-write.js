...
const { sendAdminRegistration } = require("./_admin-notification");
...
exports.handler = async (event) => {
  ...
  try {
    ...
    // Warm the local cache; the transaction still rechecks concurrent writes.
    await stateRef.once("value");
    let prevState = {};
    const result = await stateRef.transaction((current) => {
      prevState = current || {};
      const serverUpdatedAt = prevState.updatedAt || "0";
      if (expectedUpdatedAt !== undefined && expectedUpdatedAt !== serverUpdatedAt) return;
      return {
        ...nextState,
        // Ordinary clients must not overwrite the administrator's push destinations.
        adminFcmToken: prevState.adminFcmToken || null,
        deputyFcmToken: prevState.deputyFcmToken || null,
        updatedAt: String(Math.max(Date.now(), Number(serverUpdatedAt) + 1)),
      };
    });
    if (!result.committed) {
      return json(409, { ok: false, error: "동시 수정 충돌이 감지되었습니다.", conflict: true });
    }
    const savedState = result.snapshot.val();
    if (!Array.isArray(savedState.bookings)) savedState.bookings = [];
    const previousIds = new Set((prevState.bookings || []).map(b => String(b.id)));
    const newlyRegistered = (savedState.bookings || []).filter(b => !previousIds.has(String(b.id)));
    // Await delivery submission within this server invocation, independent of the browser.
    // A push failure must never turn a successfully saved booking into a failed save.
    const adminNotifications = await Promise.all(newlyRegistered.map(async (booking) => {
      try {
        return { bookingId: booking.id, ...await sendAdminRegistration(savedState.adminFcmToken, booking) };
      } catch (error) {
        console.error("admin registration push failed", booking.id, error);
        return { bookingId: booking.id, sent: false, error: "push-send-failed" };
      }
    }));

    try {
      await recordHistory(prevState.bookings, savedState.bookings);
    } catch (historyError) {
      console.error("history 기록 실패 (상태 저장은 정상 완료됨):", historyError);
    }

    return json(200, { ok: true, state: savedState, adminNotifications });
  } catch (error) {
