...
exports.handler = async (event) => {
  ...
  try {
    ...
    const reporting = (nextState.bookings || []).find(b => b.status === "REPORTING");
    const previousReporting = (prevState.bookings || []).find(b => b.status === "REPORTING");
    if (reporting && (!previousReporting || reporting.id !== previousReporting.id ||
        reporting.calledAt !== previousReporting.calledAt || reporting.fcmToken !== previousReporting.fcmToken)) {
      try {
        await require("./send-next-notification").handler({ httpMethod: "POST", body: "{}" });
      } catch (pushError) {
        console.error("Reporting push failed; queue remains saved", pushError);
      }
    }
    ...
    // Send from the server after a successful save, even if the requester closes the app.
    const urgentNotifications = [];
    for (const booking of nextState.bookings) {
      const previous = (prevState.bookings || []).find(b => b.id === booking.id);
      if (booking.status !== "WAITING" || !booking.urgentRequested || booking.isUrgent || previous?.urgentRequested) continue;
      try {
        const response = await require("./notify-admin").handler({
          httpMethod: "POST", body: JSON.stringify({ type: "urgent-request", bookingId: booking.id })
        });
        const result = JSON.parse(response.body);
        urgentNotifications.push({ bookingId: booking.id, sent: result.sent === true });
      } catch (error) {
        console.error("Urgent push failed; request remains saved", error);
        urgentNotifications.push({ bookingId: booking.id, sent: false });
      }
    }
    return json(200, { ok: true, state: nextState, urgentNotifications });
  } catch (error) {
